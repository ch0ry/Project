...
class CategoriesController{
    ...
    public async index (req: Request, res: Response){
        const products = await pool.query('SELECT * FROM proyectofinal."Product_Categories" ORDER BY super_category;');
    ...
    public async getOne(req: Request, res: Response){
        const { id } = req.params;
        let row = await pool.query('SELECT * FROM proyectofinal."Products" WHERE product_id = $1', [id]);
